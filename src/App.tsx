...
function readTransactions(): Transaction[] {
  try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]') as Transaction[] } catch { return [] }
}
...
export default function App() {
  ...
  const save = (item: Transaction) => { setTransactions(old => item.id ? old.map(value => value.id === item.id ? item : value) : [...old, { ...item, id: crypto.randomUUID() }]); setFormOpen(false) }
